...
const MostPopularAds = () => {
  const getRecentAds = async () => {
    ...
    const recentAds = adsArr.filter((ad: {isActive: boolean}) => ad.isActive).slice(0.5)
    console.log(recentAds)
  }
  getRecentAds()
    return <Box sx={{width: '300px', height: '300px', bgcolor: 'red', position: 'relative', borderRadius: '15px'}}>
     <ImageBadge width="70%" text={<Typography>$22 off</Typography>} />
    </Box>
}
